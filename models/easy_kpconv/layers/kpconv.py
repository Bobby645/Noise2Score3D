...
from models.easy_kpconv.ops.index_select import index_select
from models.easy_kpconv.layers.kpconv_utils import load_kernels
...
class KPConv(nn.Module):
    ...
    def __init__(
        ...
        # Initialize weights
        if self.groups == 1:
            weights = torch.zeros(size=(kernel_size, in_channels, out_channels)).cuda()
        else:
            weights = torch.zeros(size=(kernel_size, groups, in_channels_per_group, out_channels_per_group))
        self.weights = nn.Parameter(weights).cuda()

        if bias:
            self.bias = nn.Parameter(torch.zeros(size=(out_channels,))).cuda()
        else:
    ...
    def initialize_kernel_points(self) -> Tensor:
        ...
        return torch.from_numpy(kernel_points).float().cuda()
